...
// edit by wangyansong
double rfb::win32::Getdpi(){  //获取本机电脑分辨率
  HDC hdc = GetDC(NULL);
	int client_width = GetDeviceCaps(hdc, DESKTOPHORZRES);
	int client_height = GetDeviceCaps(hdc, DESKTOPVERTRES);
	ReleaseDC(NULL, hdc);
	// printf("系统屏幕的分辨率为：%d×%d\n", client_width, client_height);
	int nScreenWidth, nScreenHeight;
	nScreenWidth = GetSystemMetrics(SM_CXSCREEN);
	nScreenHeight = GetSystemMetrics(SM_CYSCREEN);
	// printf("当前屏幕的分辨率为：%d×%d\n", nScreenWidth, nScreenHeight);
	double dpiwidth = client_width / static_cast<double>(nScreenWidth);
	double dpiheight = client_height / static_cast<double>(nScreenHeight);
	double resdpi = (dpiwidth + dpiheight) / 2.0;
  return resdpi;
}
